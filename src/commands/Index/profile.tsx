...
export default class Profileb extends Command {
    ...
    async execute(client: Client, message: Message<boolean>, args: string[]): Promise<void> {
        ...
        let currentData = await allFetch("/link/{id}".format({ id: data.id }));
        ...
        let realResult: link = currentData.result;

        let levelUpExp: bhmoexp = (await BHMoExp("BHMoExp").where({ lv: realResult.data.level }))[0];
        let beforeExp: bhmoexp = realResult.data.level === 1 ? { lv: 0, max: 0 } : (await BHMoExp("BHMoExp").where({ lv: realResult.data.level - 1 }))[0];
 
        (levelUpExp.max - beforeExp.max) / (Number(realResult.data.mileage) - beforeExp.max);
                ...
                <>
                    <embed title={"[" + req.t("g.common.term.libra") + " ❌] " + (realResult.data.name || message.author.username.slice(0, 50))}>
                        ...
                        <field name={req.t("command.profile.health")}>
                            ...
                            <br />
                            {"> " + req.t("command.profile.healAcceleration").format({ hAccel: realResult.data.healAcceleration.toFixed(2) })}
                        </field>
                        <field name={req.t("g.common.term.strawberry")} inline={true}>
                            {"🍓 `" + Number(realResult.data.strawberry).toLocaleString() + "`"}
                        </field>
                        <field name={req.t("g.common.term.gem")} inline={true}>
                            {"💎 `" + Number(realResult.data.gem).toLocaleString() + "`"}
                        </field>
                        <field name={req.t("command.profile.buffs")} inline={true}>
                            {Object.entries(realResult.data.buffs).map(([k, v]) => {
                                return Buff[k].ko + " (" + req.t("g.common.end-in").format({ endIn: Math.round(v / 1000) }) + ")"
