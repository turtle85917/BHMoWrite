...
export default class Link extends Command {
    constructor() {
        ...
        this.interaction = async (client: Client, interaction: Interaction): Promise<void> => {
            ...
            let linkRes = res[`${interaction.customId.get("channel-id")}#${interaction.user.id}#link`];

            let guildId = interaction.customId.get("action").split("#")[1];

            let data: bhmo = (await database("bhmo").where({ guildIdExternal: guildId, userIdExternal: interaction.user.id, id: linkRes.link.id }))[0];
            if (data) {
                interaction.reply({
                    embeds: (
                        <>
            ...
            await database("bhmo").insert({
                guildIdExternal: guildId,
                userIdExternal: interaction.user.id,
                id: linkRes.link.id
            });
            ...
            await database("record").insert({
                id: linkRes.link.id,
                damage: "0"
            });

            Logger.info("Insert Table : bhmo")
            .next("guildIdExternal").put(guildId)
            .next("userIdExternal").put(interaction.user.id)
            .next("id").put(linkRes.link.id)
            .out();

            Logger.info("Insert Table : setting")
            .next("id").put(linkRes.link.id)
            .next("locale").put("ko")
            .next("writeDamage").put("y")
            .out();

            Logger.info("Insert Table : record")
            .next("id").put(linkRes.link.id)
            .next("damage").put("0")
            .out();
        }
    ...
    async execute(client: Client, message: Message<boolean>, args: string[]): Promise<void> {
        let currentData = await allFetch("/link/by-discord/{guild}/{user}".format({ guild: message.guildId, user: message.author.id }));
        ...
        let realResult: linkByDiscord = currentData.result;

        let data: bhmo = (await database("bhmo").where({ guildIdExternal: message.guildId, userIdExternal: message.author.id, id: realResult.id }))[0];
        if (data) {
            message.reply({
                embeds: (
                    <>
        ...
        .then(m => {
            let initData = { message: undefined, link: { id: realResult.id } };
